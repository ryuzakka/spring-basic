...
<div id="section">
	...
	<div id="intro">
		...
		<div id="welcome">
			<h3>비밀번호 찾기</h3>
			<span>아이디와 휴대폰 번호로 잊어버린 계정의 비밀번호를 찾으실 수 있습니다.</span>
		</div>
	</div>
	
	<div id="login-form">
		<c:if test="${param.pwd == null}">
			<form method="post" action="search_pwd_ok">
				...
				<c:if test="${param.err == 2}">
					<br><span style="color:red;font-size:13px;">탈퇴 회원입니다.</span>
				</c:if>
				<p><input type="submit" name="submit" value="비밀번호 찾기"></p>
			</form>
			<p>
			<ul id="support-btn">
		...
		<c:if test="${param.pwd != null}">
			<div id="result">찾기결과 : ${param.pwd}</div><p>
			<ul id="support-btn">
